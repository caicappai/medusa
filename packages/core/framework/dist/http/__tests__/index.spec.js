...
describe("RoutesLoader", function () {
    ...
    describe("Routes", function () {
        ...
        it("should call GET /customers/[customer_id]/orders/[order_id]", async function () {
            ...
            expect(res.status).toBe(200);
            expect(res.text).toBe('list customers {"customer_id":"test-customer","order_id":"test"}');
        });
    ...
    describe("Middlewares", function () {
        let request;
        beforeAll(async function () {
            const rootDir = (0, path_1.resolve)(__dirname, "../__fixtures__/routers-middleware");
            const { request: request_ } = await (0, server_1.createServer)(rootDir);
            request = request_;
        });
        it("should call middleware applied to `/customers`", async function () {
            ...
            expect(mocks_1.customersGlobalMiddlewareMock).toHaveBeenCalled();
        });
        it("should not call middleware applied to POST `/customers` when GET `/customers`", async function () {
            ...
            expect(res.text).toBe("list customers");
            expect(mocks_1.customersGlobalMiddlewareMock).toHaveBeenCalled();
            expect(mocks_1.customersCreateMiddlewareMock).not.toHaveBeenCalled();
        });
        it("should call middleware applied to POST `/customers` when POST `/customers`", async function () {
            ...
            expect(mocks_1.customersGlobalMiddlewareMock).toHaveBeenCalled();
            expect(mocks_1.customersCreateMiddlewareMock).toHaveBeenCalled();
        });
        it("should call store global middleware on `/store/*` routes", async function () {
            ...
            expect(mocks_1.storeGlobalMiddlewareMock).toHaveBeenCalled();
            expect(mocks_1.customersGlobalMiddlewareMock).not.toHaveBeenCalled();
            expect(mocks_1.customersCreateMiddlewareMock).not.toHaveBeenCalled();
        });
    ...
    describe("Custom error handling", function () {
        let request;
        beforeAll(async function () {
            const rootDir = (0, path_1.resolve)(__dirname, "../__fixtures__/routers-error-handler");
            const { request: request_ } = await (0, server_1.createServer)(rootDir);
            request = request_;
        });
    ...
    describe("Duplicate parameters", function () {
        ...
        it("should throw if a route contains the same parameter multiple times", async function () {
            const rootDir = (0, path_1.resolve)(__dirname, "../__fixtures__/routers-duplicate-parameter");
            const err = await new index_1.RoutesLoader({
                app,
                sourceDir: rootDir,
            })
                .load()
            ...
            expect(err.message).toBe("Duplicate parameters found in route /admin/customers/[id]/orders/[id] (id). Make sure that all parameters are unique.");
        });
    });
});
