    ...
    resolve: (serviceName) => {
        return {
            ...
            getByVariantId: (options) => {
                ...
                let orderVar = JSON.parse(JSON.stringify(data_1.remoteJoinerData.order_variant));
                if (options.expands?.order) {
                    orderVar = orderVar.map((item) => {
                        item.order = JSON.parse(JSON.stringify(data_1.remoteJoinerData.order.find((o) => o.id === item.order_id)));
                        return item;
                    });
                }
                return {
...
const fetchServiceDataCallback = jest.fn(async (expand, pkField, ids, relationship) => {
    const serviceConfig = expand.serviceConfig;
    const moduleRegistryName = !serviceConfig.serviceName.endsWith("Service")
        ? (0, utils_1.lowerCaseFirst)(serviceConfig.serviceName) + "Service"
...
describe("RemoteJoiner", () => {
    ...
    it("Simple query of a service where the returned data contains multiple properties", async () => {
        const query = joiner_1.RemoteJoiner.parseQuery(`
      query {
        product {
          id
          name
    ...
    it("It shouldn't register the service name as an alias if option autoCreateServiceNameAlias is false", async () => {
        const newJoiner = new joiner_1.RemoteJoiner(mock_data_1.serviceConfigs, fetchServiceDataCallback, { autoCreateServiceNameAlias: false });
        const query = {
            service: "user",
            fields: ["id", "name", "email"],
        };
        ...
        await expect(newJoiner.query(queryWithAlias)).rejects.toThrowError(`Service with alias "user" was not found.`);
    });
    ...
    `);
        ...
        const queryNotFound = joiner_1.RemoteJoiner.parseQuery(`
      query {
    ...
    `);
        ...
        await expect(dataNotFound).rejects.toThrowError("order id not found: ord_1234556");
    });
});
//# sourceMappingURL=remote-joiner-data.js.map