...
describe("Transaction Orchestrator", () => {
    ...
    it("Should follow the flow by calling steps in order with the correct payload", async () => {
        ...
        async function handler(actionId, functionHandlerType, payload) {
            const command = {
                firstMethod: {
                    [transaction_1.TransactionHandlerType.INVOKE]: () => {
                        mocks.one(payload);
                    },
                },
                secondMethod: {
        ...
        const strategy = new transaction_1.TransactionOrchestrator({
            id: "transaction-name",
            definition: flow,
        });
        ...
        expect(transaction.getState()).toBe(transaction_1.TransactionState.DONE);
        expect(mocks.one).toBeCalledWith(expect.objectContaining({
            metadata: {
                model_id: "transaction-name",
                idempotency_key: "transaction-name:transaction_id_123:firstMethod:invoke",
                action: "firstMethod",
                action_type: "invoke",
                attempt: 1,
        ...
        expect(mocks.two).toBeCalledWith(expect.objectContaining({
            metadata: {
                model_id: "transaction-name",
                idempotency_key: "transaction-name:transaction_id_123:secondMethod:invoke",
                action: "secondMethod",
                action_type: "invoke",
                attempt: 1,
                timestamp: expect.any(Number),
    ...
    it("Should not execute next steps when a step fails", async () => {
        const actionOrder = [];
        async function handler(actionId, functionHandlerType, payload) {
            if (functionHandlerType === transaction_1.TransactionHandlerType.INVOKE) {
                actionOrder.push(actionId);
            }
            if (transaction_1.TransactionHandlerType.INVOKE && actionId === "three") {
                throw new Error();
            }
        }
        const flow = {
    ...
    it("Should store invoke's step response by default or if flag 'saveResponse' is set to true and ignore it if set to false", async () => {
        ...
        async function handler(actionId, functionHandlerType, payload) {
            const command = {
                firstMethod: {
                    [transaction_1.TransactionHandlerType.INVOKE]: (data) => {
                ...
                thirdMethod: {
                    [transaction_1.TransactionHandlerType.INVOKE]: (data, context) => {
            ...
            return command[actionId][functionHandlerType](payload.data, payload.context);
        }
        const flow = {
            next: {
                action: "firstMethod",
        ...
        const transaction = await strategy.beginTransaction("transaction_id_123", handler, { prop: 123 });
        await strategy.resume(transaction);
        expect(mocks.one).toBeCalledWith({ prop: 123 });
        expect(mocks.two).toBeCalledWith({ prop: 123 });
        expect(mocks.three).toBeCalledWith({ prop: 123 }, {
            payload: {
                prop: 123,
            },
            invoke: {
    ...
    it("Should store compensate's step responses if flag 'saveResponse' is set to true", async () => {
        ...
        async function handler(actionId, functionHandlerType, payload) {
            const command = {
                firstMethod: {
                    ...
                    [transaction_1.TransactionHandlerType.COMPENSATE]: ({ compensate }) => {
        ...
        expect(resposes.compensate.firstMethod).toEqual("compensate 1 - 2 = isCompensated");
    });
    ...
    it("Should retry steps X times when a step fails and compensate steps afterward", async () => {
        ...
        async function handler(actionId, functionHandlerType, payload) {
            const command = {
                firstMethod: {
                    ...
                    [transaction_1.TransactionHandlerType.COMPENSATE]: () => {
        ...
        expect(transaction.getState()).toBe(transaction_1.TransactionState.REVERTED);
        expect(mocks.compensateOne).toHaveBeenCalledTimes(1);
        expect(mocks.two).nthCalledWith(1, expect.objectContaining({
            metadata: expect.objectContaining({
                attempt: 1,
            }),
        }));
        expect(mocks.two).nthCalledWith(4, expect.objectContaining({
            metadata: expect.objectContaining({
                attempt: 4,
            }),
        }));
    ...
    it("Should fail a transaction if any step fails after retrying X time to compensate it", async () => {
        ...
        expect(transaction.getState()).toBe(transaction_1.TransactionState.FAILED);
    });
    ...
    it("Should hold the status INVOKING while the transaction hasn't finished", async () => {
        ...
        expect(transaction.getState()).toBe(transaction_1.TransactionState.INVOKING);
        expect(transaction.getFlow().hasWaitingSteps).toBe(true);
        const mocktransactionId = transaction_1.TransactionOrchestrator.getKeyName("transaction-name", transaction.transactionId, "firstMethod", transaction_1.TransactionHandlerType.INVOKE);
        await strategy.registerStepSuccess(mocktransactionId, undefined, transaction);
        expect(transaction.getState()).toBe(transaction_1.TransactionState.DONE);
    ...
    it("Should hold the status COMPENSATING while the transaction hasn't finished compensating", async () => {
        ...
        const transaction = await strategy.beginTransaction("transaction_id_123", handler);
        const mocktransactionId = transaction_1.TransactionOrchestrator.getKeyName("transaction-name", transaction.transactionId, "firstMethod", transaction_1.TransactionHandlerType.INVOKE);
        const mockSecondStepId = transaction_1.TransactionOrchestrator.getKeyName("transaction-name", transaction.transactionId, "secondMethod", transaction_1.TransactionHandlerType.INVOKE);
        await strategy.resume(transaction);
        expect(mocks.one).toHaveBeenCalledTimes(1);
        ...
        const registerBeforeAllowed = await strategy
            ...
            .catch((e) => e.message);
        expect(registerBeforeAllowed).toEqual("Cannot set step success when status is idle");
        expect(transaction.getState()).toBe(transaction_1.TransactionState.INVOKING);
        const resumedTransaction = await strategy.registerStepFailure(mocktransactionId, null, handler);
        expect(resumedTransaction.getState()).toBe(transaction_1.TransactionState.COMPENSATING);
        expect(mocks.compensateOne).toHaveBeenCalledTimes(1);
        const mocktransactionIdCompensate = transaction_1.TransactionOrchestrator.getKeyName("transaction-name", transaction.transactionId, "firstMethod", transaction_1.TransactionHandlerType.COMPENSATE);
        await strategy.registerStepSuccess(mocktransactionIdCompensate, undefined, resumedTransaction);
        expect(resumedTransaction.getState()).toBe(transaction_1.TransactionState.REVERTED);
    });
    ...
    it("Should receive the current transaction as reference in the handler", async () => {
        ...
        async function handler(actionId, functionHandlerType, payload, transaction) {
    ...
    describe("Timeouts - Transaction and Step", () => {
        it("should fail the current steps and revert the transaction if the Transaction Timeout is reached", async () => {
            ...
            async function handler(actionId, functionHandlerType, payload) {
                const command = {
                    ...
                    action2: {
                        [transaction_1.TransactionHandlerType.INVOKE]: async () => {
            ...
            expect(transaction.getErrors()[0].error).toBeInstanceOf(transaction_1.TransactionTimeoutError);
        ...
        it("should continue the transaction and skip children steps when the Transaction Step Timeout is reached but the step is set to 'continueOnPermanentFailure'", async () => {
            ...
            expect(transaction.getContext().invoke.action3).toBe("content f3");
            expect(transaction.getContext().invoke.action4).toBe(undefined);
            expect(transaction.getFlow().steps["_root.action1.action2"].invoke.state).toBe(utils_1.TransactionStepState.TIMEOUT);
            expect(transaction.getFlow().steps["_root.action1.action2"].invoke.status).toBe(utils_1.TransactionStepStatus.PERMANENT_FAILURE);
            expect(transaction.getFlow().steps["_root.action1.action2"].compensate.state).toBe(utils_1.TransactionStepState.DORMANT);
            expect(transaction.getFlow().steps["_root.action1.action2.action4"].invoke
                .state).toBe(utils_1.TransactionStepState.SKIPPED_FAILURE);
            expect(transaction.getFlow().steps["_root.action1.action2.action4"].invoke
                .status).toBe(utils_1.TransactionStepStatus.IDLE);
            expect(transaction.getState()).toBe(transaction_1.TransactionState.DONE);
        });
        it("should fail the current steps and revert the transaction if the Step Timeout is reached", async () => {
            ...
            expect(transaction.getContext().invoke.action3).toBe("content f3");
            expect(transaction.getContext().invoke.action4).toBe(undefined);
            expect(transaction.getErrors()[0].error).toBeInstanceOf(transaction_1.TransactionStepTimeoutError);
            expect(transaction.getErrors()[0].action).toBe("action2");
            expect(transaction.getState()).toBe(transaction_1.TransactionState.REVERTED);
        });
        it("should fail the current steps and revert the transaction if the Transaction Timeout is reached event if the step is set as 'continueOnPermanentFailure'", async () => {
            ...
            expect(transaction_1.TransactionTimeoutError.isTransactionTimeoutError(transaction.getErrors()[0].error)).toBe(true);
            expect(transaction.getErrors()[0].action).toBe("action2");
            expect(transaction_1.TransactionTimeoutError.isTransactionTimeoutError(transaction.getErrors()[1].error)).toBe(true);
