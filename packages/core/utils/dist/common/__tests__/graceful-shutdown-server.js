...
const graceful_shutdown_server_1 = require("../graceful-shutdown-server");
describe("GracefulShutdownServer", () => {
    ...
    it('should add "isShuttingDown" property to the existing server', () => {
        const server = graceful_shutdown_server_1.GracefulShutdownServer.create({ on: jest.fn() });
    ...
    it("should listen for client connections and store reference to them", async () => {
        ...
        graceful_shutdown_server_1.GracefulShutdownServer.create({ on: onEventMock });
        expect(onEventMock).toBeCalledTimes(3);
        expect(onEventMock.mock.calls[2][0]).toEqual("request");
        const connectEvent = onEventMock.mock.calls[0][1];
        ...
        expect(socket2).toEqual(expect.objectContaining({
            _idle: true,
            _connectionId: 2,
        }));
        ...
        expect(socket3).toEqual(expect.objectContaining({
            _idle: false,
            _connectionId: 3,
        }));
        finishRequestMock();
    ...
    it("waits requests to complete before shutting the server down", (done) => {
        jest.useFakeTimers();
        const onEventMock = jest.fn();
        ...
        const waitTime = 200;
        let closeServerCallback;
        const server = graceful_shutdown_server_1.GracefulShutdownServer.create({
            close: (callback) => {
    ...
    it("should force close all connections after the timeout is reached", (done) => {
        jest.useFakeTimers();
        const onEventMock = jest.fn();
        const setIntervalSpy = jest.spyOn(global, "setInterval");
        const setTimeoutSpy = jest.spyOn(global, "setTimeout");
