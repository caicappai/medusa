...
const mikro_orm_create_connection_1 = require("../mikro-orm-create-connection");
...
describe("CustomTsMigrationGenerator", () => {
    it('should add "if not exists" to "create table" statements', () => {
        ...
        const result = unwrapSql(mikro_orm_create_connection_1.CustomTsMigrationGenerator.prototype.createStatement(sql, 0));
    ...
    it('should add "if exists" to "alter table" statements as well as to the add column', () => {
        ...
        const result = unwrapSql(mikro_orm_create_connection_1.CustomTsMigrationGenerator.prototype.createStatement(sql, 0));
        expect(result).toBe("alter table if exists my_table add column if not exists name varchar(100)");
    });
    it('should add "if exists" to "alter table" statements as well as to the drop column', () => {
        ...
        const result = unwrapSql(mikro_orm_create_connection_1.CustomTsMigrationGenerator.prototype.createStatement(sql, 0));
        expect(result).toBe("alter table if exists my_table drop column if exists name");
    });
    ...
    it('should add "if not exists" to "create unique index" statements', () => {
        ...
        const result = unwrapSql(mikro_orm_create_connection_1.CustomTsMigrationGenerator.prototype.createStatement(sql, 0));
        expect(result).toBe("create unique index if not exists idx_unique_name on my_table(name)");
    });
...
//# sourceMappingURL=ts-migration-generator.spec.js.map