 ...
 * entity
 */
function defineHasOneWithFKRelationship(MikroORMEntity, relationship, { relatedModelName }, cascades) {
    const foreignKeyName = relationship.options.foreignKeyName ??
        (0, common_1.camelToSnakeCase)(`${relationship.name}Id`);
    const shouldRemoveRelated = !!cascades.delete?.includes(relationship.name);
    let mappedBy = (0, common_1.camelToSnakeCase)(MikroORMEntity.name);
    if ("mappedBy" in relationship) {
        mappedBy = relationship.mappedBy;
...
function defineRelationship(MikroORMEntity, entity, relationship, cascades, context) {
    ...
    switch (relationship.type) {
        case "hasOne":
            ...
            break;
        case "hasOneWithFK":
            defineHasOneWithFKRelationship(MikroORMEntity, relationship, relatedEntityInfo, cascades);
            break;
        case "hasMany":
