...
describe("Internal Module Service Factory", () => {
    const modelRepositoryName = `${(0, common_1.lowerCaseFirst)(Model.name)}Repository`;
    const containerMock = {
        [modelRepositoryName]: {
    ...
    const IMedusaInternalService = (0, medusa_internal_service_1.MedusaInternalService)(Model);
    describe("Internal Module Service Methods", () => {
        ...
        it("should throw an error on retrieve if composite key values are not defined", async () => {
            ...
            CompositeModel.meta = { primaryKeys: ["id", "name"] };
            const compositeIMedusaInternalService = (0, medusa_internal_service_1.MedusaInternalService)(CompositeModel);
        ...
        it("should retrieve entity successfully with composite key", async () => {
            ...
            const instance = new compositeIMedusaInternalService(containerMock);
            const entity = { id: "1", name: "Item" };
            containerMock[`${(0, common_1.lowerCaseFirst)(CompositeModel.name)}Repository`].find.mockResolvedValueOnce([entity]);
        ...
        it("should list entities and relation with default ordering successfully", async () => {
            ...
            expect(result).toEqual(entities);
            expect(containerMock[modelRepositoryName].find).toHaveBeenCalledWith(expect.objectContaining({
        ...
        it("should create entity successfully", async () => {
            ...
            containerMock[modelRepositoryName].create.mockImplementation(async (entity) => entity);
        ...
        it("should update entities successfully", async () => {
            ...
            containerMock[modelRepositoryName].find.mockResolvedValueOnce(entitiesToUpdate);
            containerMock[modelRepositoryName].update.mockResolvedValueOnce([
        ...
        it("should delete entities successfully", async () => {
            ...
            containerMock[modelRepositoryName].find.mockResolvedValueOnce(entitiesToDelete);
        ...
        it("should soft delete entity successfully", async () => {
            ...
            expect(containerMock[modelRepositoryName].softDelete).toHaveBeenCalledWith("1", defaultContext);
        });
        it("should prevent from soft deleting all data", async () => {
            await instance.softDelete([]);
            expect(containerMock[modelRepositoryName].softDelete).not.toHaveBeenCalled();
        });
        it("should restore entity successfully", async () => {
            await instance.restore("1");
            expect(containerMock[modelRepositoryName].restore).toHaveBeenCalledWith("1", defaultContext);
        });
    });
});
