...
import { extname, join, resolve as resolve3 } from "path";
...
async function generateIndex(path2) {
  let index = BANNER;
  const entries = await fse.readdir(path2);
  ...
  await fse.writeFile(resolve2(path2, "index.ts"), index);
}
...
var PREFIX = colors.magenta("[toolbox]");
var logger = {
  info: (message) => {
...
async function generateIcons({ output }) {
  ...
  for (let i = 0; i < length; i += 20) {
    ...
    const requests = slice.map(async (icon) => {
      ...
      const filePath = resolve3(output, fileName);
      await fse2.outputFile(filePath, component);
      const ext = extname(fileName);
      const fileNameWithoutExt = fileName.replace(ext, "");
      const testFilePath = resolve3(join(output, "__tests__"), testName);
      const testFile = dedent2`
        import * as React from "react"
      ...
      `;
      await fse2.outputFile(testFilePath, testFile);
    });
